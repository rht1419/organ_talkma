...
const SITE_URL = 'http://localhost:5173'; // Optional: for OpenRouter rankings
const SITE_NAME = 'Organ Talkma'; // Optional: for OpenRouter rankings
...
/**
 * Generates a response using OpenRouter AI
...
export async function generateResponse(userMessage, mode, character, conversationHistory = []) {
  try {
    const systemPromptText = mode === 'organs' 
      ? characterPrompts?.[character] || characterPrompts?.heart
      : lifestylePrompt;

    const responseRequest = mode === 'lifestyle' 
        ? 'Provide a comprehensive, detailed response with specific advice, practical tips, and explanations. Aim for 4-6 sentences with rich, actionable content.' 
        : 'Limit response to 2-3 sentences for natural conversation flow.';
    ...
    const fallbackResponses = {
      organs: {
        ...
        brain: "Neural pathways are momentarily blocked. 🧠 Please standby for reconnection."
      },
      lifestyle: "I'm having some technical difficulties right now, but I'm here to help you with your wellness journey! Please try your question again in a moment."
    };
...
export async function generateWelcomeMessage(mode, character) {
  try {
    ...
    const welcomeLength = mode === 'lifestyle'
      ? 'Create a comprehensive, warm welcome message that introduces your expertise and expresses enthusiasm about helping with their wellness journey. Include a brief overview of what you can help with and invite them to share their goals. Aim for 3-4 sentences.' 
      : 'Generate a brief, engaging welcome message as if you\'re meeting this person for the first time. Keep it to 1-2 sentences and stay in character.';
    
    const messages = [
        { role: 'system', content: systemPromptText },
        { role: 'user', content: `Generate a welcome message. ${welcomeLength}` }
    ];
    ...
    const fallbackWelcomes = {
      organs: {
        ...
        brain: "Hello there! 🧠 Your brain speaking - the command center of your entire being! I'm excited to help you optimize your cognitive health."
      },
      lifestyle: "Hello! I'm your comprehensive lifestyle and wellness coach, here to help you build sustainable healthy habits. What aspect of your wellness journey would you like to focus on today?"
    };
